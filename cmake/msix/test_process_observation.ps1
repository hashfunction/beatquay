...
foreach ($code in @(0,7)) {
    ...
    try {
        ...
        $timeout=Get-BeatQuayProcessExitEvidence $attached 1
        ...
        $exit=Get-BeatQuayProcessExitEvidence $attached 10000
...
try {
    $observed=Get-BeatQuayProcessExitEvidence $unattached 1
...
foreach ($owned in @($false,$true)) {
    $directory=Join-Path ([IO.Path]::GetTempPath()) ('beatquay-process-ownership-'+[guid]::NewGuid().ToString('N'))
    ...
    try {
        ...
        try { Invoke-BeatQuayInstallQualification unused unused unused $directory | Out-Null } catch {
