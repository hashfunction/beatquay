...
$script:probeRoot=Join-Path ([IO.Path]::GetTempPath()) ('beatquay-temporary-test-'+[guid]::NewGuid().ToString('N'))
...
try {
    $failure=$null
    try { Invoke-BeatQuayInstallQualification unused unused unused $script:probeRoot | Out-Null }
