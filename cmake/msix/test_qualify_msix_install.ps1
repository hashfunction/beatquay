...
$insidePackage = Test-PathInside -Candidate (Join-Path $packageRoot 'lmms.exe') -Root $packageRoot
...
$recordFixture = [pscustomobject]@{ payload = [pscustomobject]@{ 'lmms.exe' = [pscustomobject]@{ bytes=1; sha256=('a' * 64) } } }
Assert-True ((Get-RecordPayloadEntry $recordFixture 'lmms.exe').bytes -eq 1) 'slash-qualified payload property must resolve exactly'
