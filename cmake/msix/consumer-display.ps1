...
function Add-BeatQuayConsumerDisplayTypes {
    if('BeatQuayConsumerDisplay.DisplayModes' -as [type]){return}
    Add-Type -TypeDefinition @'
using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Runtime.InteropServices;
namespace BeatQuayConsumerDisplay {
...
function Convert-BeatQuayConsumerMode($Mode){
...
function Get-BeatQuayConsumerModeChoice($Modes){
...
function Get-BeatQuayConsumerDisplayState([string]$Device){
    Add-BeatQuayConsumerDisplayTypes
    if(-not $Device){$Device=[BeatQuayConsumerDisplay.DisplayModes]::Primary()}
    return @{device=$Device;current=[BeatQuayConsumerDisplay.DisplayModes]::Current($Device);modes=[BeatQuayConsumerDisplay.DisplayModes]::Supported($Device)}
}
function Set-BeatQuayConsumerDisplayMode([string]$Device,$Mode,[int]$Flags){
    return [BeatQuayConsumerDisplay.DisplayModes]::Change($Device,$Mode,$Flags)
}

function Start-BeatQuayConsumerDisplay($State){
    $snapshot=Get-BeatQuayConsumerDisplayState
    $State.displayOriginalMode=$snapshot.current;$State.displayDevice=$snapshot.device
    $State.displayEvidence=[ordered]@{purpose='actual native display only';device=$snapshot.device;before=(Convert-BeatQuayConsumerMode $snapshot.current);
        supported_modes=@($snapshot.modes|ForEach-Object {Convert-BeatQuayConsumerMode $_});selected=$null;test_result=$null;apply_result=$null;after=$null;
    ...
    if($snapshot.current.dmPelsWidth -ge 1472 -and $snapshot.current.dmPelsHeight -ge 1080 -and $snapshot.current.dmBitsPerPel -eq 32){
        $State.displayEvidence.after=Convert-BeatQuayConsumerMode $snapshot.current;return
    }
    $selected=Get-BeatQuayConsumerModeChoice $snapshot.modes;$State.displayEvidence.selected=Convert-BeatQuayConsumerMode $selected
    $test=Set-BeatQuayConsumerDisplayMode $snapshot.device $selected 2;$State.displayEvidence.test_result=$test
    ...
    $apply=Set-BeatQuayConsumerDisplayMode $snapshot.device $selected 0;$State.displayEvidence.apply_result=$apply
    if($apply -ne 0){throw "Native dynamic display change failed ($apply)."}
    $deadline=[DateTime]::UtcNow.AddSeconds(5)
    do{
        $after=(Get-BeatQuayConsumerDisplayState $snapshot.device).current
    ...
    $State.displayEvidence.after=Convert-BeatQuayConsumerMode $after
...
function Restore-BeatQuayConsumerDisplay($State){
    if(-not $State.displayOriginalMode){return}
    if($State.displayRestoreRequired){
        $result=Set-BeatQuayConsumerDisplayMode $State.displayDevice $State.displayOriginalMode 0;$State.displayEvidence.restore_result=$result
        if($result -ne 0){throw "Native display restoration failed ($result)."}
    }
    $deadline=[DateTime]::UtcNow.AddSeconds(5)
    do{
        $observed=Convert-BeatQuayConsumerMode (Get-BeatQuayConsumerDisplayState $State.displayDevice).current
