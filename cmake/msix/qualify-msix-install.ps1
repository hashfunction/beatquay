# Disposable Windows CI installation qualification for BeatQuay.
...
function Get-BeatQuayProcessExitEvidence([Diagnostics.Process]$Process, [int]$TimeoutMilliseconds) {
...
function Add-BeatQuayActivationTypes {
    if ('BeatQuayQualification.NativePackageProbe' -as [type]) { return }
...
function Assert-BeatQuayWindowEvidence($Snapshot) {
    ...
    if (-not $Snapshot.actionable_controls_verified -or $Snapshot.actionable_control_count -lt 1) {
        throw 'The owned BeatQuay editor has no visible enabled controls.'
    }
}
...
function Get-WindowQualification([Diagnostics.Process]$Process, [string]$OutputDirectory) {
    ...
    try {
        ...
        [BeatQuayQualification.NativePackageProbe]::ShowWindow($Process.MainWindowHandle,9) | Out-Null
        [BeatQuayQualification.NativePackageProbe]::SetForegroundWindow($Process.MainWindowHandle) | Out-Null
        Start-Sleep -Milliseconds 250
        [uint32]$foregroundPid=0
        [BeatQuayQualification.NativePackageProbe]::GetWindowThreadProcessId(
            [BeatQuayQualification.NativePackageProbe]::GetForegroundWindow(),[ref]$foregroundPid) | Out-Null
    ...
    Assert-BeatQuayWindowEvidence $snapshot
    return $snapshot
}
...
function Invoke-BeatQuayInstallQualification([string]$PackagePath, [string]$RecordPath, [string]$SignToolPath, [string]$OutputPath) {
    ...
    $operations.PrepareSignedCopy = {
        ...
        $temporaryCandidate = Join-Path $runnerTemp ('.beatquay-install-' + [guid]::NewGuid().ToString('N'))
