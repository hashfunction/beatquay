...
foreach($fixture in @('test_qualify_msix_install.ps1','test_appx_preflight.ps1','test_msix_evidence.ps1','test_registration_ownership.ps1','test_process_observation.ps1','test_window_evidence.ps1','test_first_run_flow.ps1','test_defender_module.ps1','test_temporary_ownership.ps1','test_consumer_workflow.ps1','test_consumer_display.ps1')){Invoke-Checked $powerShell @('-NoLogo','-NoProfile','-File',(Join-Path $PSScriptRoot $fixture))}
...
Invoke-Checked $powerShell @('-NoLogo','-NoProfile','-File','cmake/msix/qualify-msix-install.ps1','-Package',(Join-Path $packageOutput 'BeatQuay.Qualification_1.0.0.0_x64.msix'),'-PackageRecord',(Join-Path $packageOutput 'package-record.json'),'-SignTool',(Join-Path $sdkDirectory 'signtool.exe'),'-Output','build-evidence/msix-install','-Python',$pythonPath)
